...
- types `$mdurl` or `/mdurl <path>` explicitly
...
> ```
> After that, each user's Claude/Codex skill symlinks come from running `bash ~/Coding/0_agents/update.sh` once under their account.
